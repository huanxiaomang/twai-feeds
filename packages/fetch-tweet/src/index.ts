...
export async function fetchAndFormat(options: FetchOptions) {
    ...
    // Task A: Translation (可控)
    if (options.enable_translation !== false) {
        const translationTask = (async () => {
            ...
            console.log(`AI translation finished. Processed ${translatedResults.length} items.`);
        ...
        console.log('Translation skipped (disabled in config)');
    }

    // Task B: Tags (可控)
    let generatedTagsInfo: { tag_id: string; tag_name: string }[] = [];
    if (options.enable_tags !== false) {
        ...
        console.log('Tags generation skipped (disabled in config)');
        // 可选 fallback 到 config 预定义标签
        if ((options.feed_info as any).tags_info) {
            generatedTagsInfo = (options.feed_info as any).tags_info;
        }
    }
...
if (process.argv[1] === import.meta.url || process.argv[1]?.endsWith('index.ts') || process.argv[1]?.endsWith('index.mjs') || process.argv[1]?.endsWith('index.cjs')) {
    (async () => {
        try {
            ...
            // 读取开关，默认全部启用
            const enableTranslation = config.enable_translation !== false;
            const enableTags = config.enable_tags !== false;
            const enableOverview = config.enable_overview !== false;
            ...
            // Save AI Bots Content
            if (Object.keys(feed_item_ai_bots_content).length > 0) {
            ...
            // Save Overview (受 enable_overview 控制)
            if (enableOverview && feed.list && feed.list.length > 0) {
                ...
                if (overviewResult.overview && Object.keys(overviewResult.overview).length > 0) {
                ...
                console.log('Overview generation skipped (disabled in config)');
            }
